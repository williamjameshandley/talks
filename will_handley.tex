...
\begin{frame}
  ...
  \resizebox{\textwidth} {!} {%
    ...
    % Draw axes
      \draw<1-5> [<->,thick] (0,\ywidth) node (yaxis) [above] {$\log\mathcal{P}_\mathcal{R}(k)$}
    ...
    % Draw the line joining start and end
      ...
      \draw<2> (mn) -- (mx);
      \draw<2-5> (mn) node[below right]    {$(k_1,\mathcal{P}_{1})$};
      \draw<2> (mx) node[below left]     {$(k_{2},\mathcal{P}_{{2}})$};
      \onslide<2-5>{\movablevert{mn}};
      \onslide<2-5>{\movablevert{mx}};

      \draw<3> (mn) -- (start) -- (mx);
      \onslide<3-5>{\movablecross{start}};
      \draw<3-5> (start) node[above right] {$(k_2,\mathcal{P}_{2})$};
      \draw<3> (mx) node[below left]     {$(k_{3},\mathcal{P}_{{3}})$};
 
      \draw<4> (mn) -- (start) -- (mid) -- (mx);
      \onslide<4-5>{\movablecross{mid}};
      \draw<4-5> (mid) node[below right] {$(k_3,\mathcal{P}_{3})$};
      \draw<4> (mx) node[below left]     {$(k_{4},\mathcal{P}_{{4}})$};

      \draw<5> (mn) -- (start) -- (mid) -- (end) -- (mx);
      \onslide<5>{\movablecross{end}};
      \draw<5> (end) node[above right] {$(k_4,\mathcal{P}_{4})$};
      \draw<5> (mx) node[below left]     {$(k_{N_\text{knots}},\mathcal{P}_{N_\text{knots}})$};
    ...
    % Draw the point labels
      %\draw<2-> (mn) node[below right]    {$(k_1,\mathcal{P}_{1})$};
      %\draw<2-> (start) node[above right] {$(k_2,\mathcal{P}_{2})$};
      %\draw<2-> (mid) node[below right]   {$(k_3,\mathcal{P}_{3})$};
      %\draw<2-> (end) node[above right]   {$(k_4,\mathcal{P}_{4})$};
      %\draw<2-> (mx) node[below left]     {$(k_{N_\text{knots}},\mathcal{P}_{N_\text{knots}})$};
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item Gives a specific absorption trough
            \item $T_\text{sky}(\Omega,\nu) = T_\text{global}(\nu) + T_\text{foreground}(\Omega,\nu) + \text{Noise}$
            \item Measuring the first stars in the universe turning on with christmas trees in the desert.
        \end{itemize}
...
\begin{frame}
    ...
    \begin{columns}
        \column{0.6\textwidth}
        \begin{itemize}
            \item EDGES~[\href{https://www.nature.com/articles/nature25792}{Nat}] claimed a very controversial detection
            \item SARAS3~[\href{https://www.nature.com/articles/s41550-022-01610-5}{NatAstro}]
            \item REACH~[\href{https://www.nature.com/articles/s41550-022-01709-9}{NatAstro} aims to add to this question
            \item A collaboration truly powered by nested sampling.
        ...
        \includegraphics[height=0.33\textwidth]{figures/EDGES_antenna}
        \includegraphics[height=0.33\textwidth]{figures/SARAS}
...
\begin{frame}
    \frametitle{Statistics: fast estimation of small $p$-values~\arxiv{2106.02056}}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Convergent Bayesian Global Fits of 4D Composite Higgs Models~\arxiv{2101.00428}
        \item Thermal WIMPs and the Scale of New Physics: Global Fits of Dirac Dark Matter Effective Field Theories~\arxiv{2205.13549}
    \end{itemize}
