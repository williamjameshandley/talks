...
\begin{frame}
    ...
    \only<1>{%
        \begin{columns}
            \column{0.5\textwidth}
            \begin{overpic}[width=\textwidth]{figures/inflation}
                \put(85,5) {\tiny\arxiv{1807.06211}}
            \end{overpic}
            \column{0.5\textwidth}
            \begin{overpic}[width=\textwidth]{figures/axions}
                \put(70,2) {\tiny\github{cajohare/AxionLimits}}
            \end{overpic}
        \end{columns}
        \begin{columns}
            \column{0.5\textwidth}
            \begin{itemize}
                \item Contours indicate \textbf{allowed} regions
                \item Preferred in astro/cosmology
            \end{itemize}
            \column{0.5\textwidth}
            \begin{itemize}
                \item Contours indicate \textbf{excluded} regions
                \item Preferred in particle physics
            \end{itemize}
        \end{columns}
    }%
    \only<2>{%
        \begin{columns}
            \column{0.4\textwidth}
            \begin{itemize}
                \item Beware this kind of particularly confusing plot, which uses both!
                \item Here almost all of these are $2\sigma$ exclusion plots
                \item But 'DAMA' are (controversial \& conflicting) superimposed constraints/allowed regions.
            \end{itemize}
            \column{0.6\textwidth}
            \begin{overpic}[width=\textwidth]{figures/combined}
                \put(55,73){\tiny\github{cajohare/DirectDetectionPlots}}
            \end{overpic}
        \end{columns}
    }
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item Present state of the art achieves this using \emph{machine learning} (neural networks).
                \begin{itemize}
                    \item My group's research tries to removes machine learning \github{handley-lab/lsbi}.
                \end{itemize}
        \end{itemize}
        \column{0.5\textwidth}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \github{florent-leclercq/pyselfi}
        \includegraphics[width=\textwidth]{figures/delfi_screenshot}
        \github{justinalsing/pydelfi}
    \end{columns}
\end{frame}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item \github{smsharma/awesome-neural-sbi}
        \end{itemize}
        \column{0.3\textwidth}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{block}{the \texttt{swyft} package}
            ...
            \end{itemize}
            {\small \tthref{swyft.readthedocs.io/en/stable}}
        \end{block}
        \begin{block}{the \texttt{pydelfi} package}
            \begin{itemize}
                ...
                \item astro/cosmology specific examples
            \end{itemize}
            {\small \tthref{justinalsing.github.io/pydelfi}}
        \end{block}
        \column{0.45\textwidth}
        \begin{block}{the \texttt{sbi} package}
            ...
            \end{itemize}
            {\small \tthref{sbi-dev.github.io/sbi/latest/tutorials}}
        \end{block}
...
\begin{frame}
    ...
    \begin{columns}
        \column{0.45\textwidth}
        \begin{description}
            \item[CMB] \texttt{cosmopower}~\arxiv{2106.03846}
            \item[CMB] \texttt{candl}~\arxiv{2401.13433}
            \item[SNe] \texttt{BayesSN}~\arxiv{2401.08755}
            \item[SGW]  \texttt{Eryn}~\arxiv{2303.02164}
            \item[GW] \texttt{redback}~\arxiv{2308.12806}
            \item[GW] \texttt{ripple}~\arxiv{2302.05329}
            \item[EP] \texttt{ExoJAX}~\arxiv{2105.14782}
            \item[X]  \texttt{jaxspec}~\arxiv{2409.05757}
        \end{description}
        \github{JAXtronomy}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        ...
        \item Have implemented a nested slice sampler in \texttt{blackjax}~\github{blackjax-devs/blackjax/pull/755}.
            \begin{lstlisting}[language=Python]
            pip install git+https://github.com/handley-lab/blackjax@nested_sampling
            import blackjax.ns.adaptive\end{lstlisting}
        \item Think \texttt{MultiNest} for jax.
        \item Plugs into \texttt{jim}~\github{kazewong/jim} and \texttt{ripple}~\arxiv{2302.05329}
    \end{itemize}
\end{frame}
