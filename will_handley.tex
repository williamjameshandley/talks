...
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{tikzpicture}[node distance=1cm, every neuron/.style={circle, draw, minimum size=1cm},]
            ...
            \node[every neuron/.try] (logr) { $\ln r$};
            \node[every neuron/.try, left= 0.5cm of logr] (p) { $p$};

            \node[left of=p] (hgap)  {\vdots};
            \node[every neuron/.try, above of=hgap] (h1) {};
            \node[every neuron/.try, below of=hgap] (hk)  {};

            \node[left = 1cm of hgap] (jgap)  {\vdots};
            \node[every neuron/.try, above of=jgap] (j1) {};
            \node[every neuron/.try, below of=jgap] (jk)  {};

            \node[every neuron/.try, above left= 0cm and 1cm of jgap] (thetan)  { $\theta_n$};
            \node[above of=thetan] (thetagap)  {\vdots};
            \node[every neuron/.try, above of= thetagap] (theta1) { $\theta_1$};

            \node[every neuron/.try, below left = 0cm and 1cm of jgap] (D1)  { $D_1$};
            \node[below of = D1] (Dgap)  {\vdots};
            \node[every neuron/.try, below of=Dgap] (Dd)  { $D_d$};
