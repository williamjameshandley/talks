...
\begin{frame}
    \frametitle{BlackJAX: GPU Native Sampling \arxiv{2402.10797}}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{block}{Cosmic Shear (37 params)}
            ...
            \end{itemize}
            \begin{center}
\textbf{Significant improvement over previous GPU methods}~\arxiv{2405.12965}
            \end{center}
        \end{block}
        \column{0.48\textwidth}
...
\begin{frame}
    \frametitle{GPU-Accelerated Nested Sampling for Cosmology \arxiv{2509.13307}}
    \student{toby_lovick}{Toby Lovick}{PhD}
    \begin{columns}
        \column{0.48\textwidth}
        \begin{block}{High-Dimensional Model Comparison}
            \begin{itemize}
                \item JAX-based neural emulators + NS
                \item Direct Bayesian evidence calculation
                \item 39-dimensional $\Lambda$CDM vs $w_0w_a$
            \end{itemize}
        \end{block}
        \begin{block}{Performance Results}
            \begin{itemize}
                \item \textbf{Single A100 GPU}: 2 days
                \item \textbf{CPU (48 cores)}: months
                \item Produces Bayes Factors with robust errors
                \item No accuracy loss vs CPU methods
            \end{itemize}
        \end{block}
        \column{0.48\textwidth}
        \begin{block}{Key Innovation}
            \begin{itemize}
                \item Nested sampling competitive with MCMC+evidence methods
                \item Vectorized likelihood evaluations
                \item Neural network emulators for speed
                \item Direct model comparison capability
            \end{itemize}
        \end{block}
        \begin{block}{Implications}
            GPU acceleration puts nested sampling on equal computational footing with state-of-the-art methods, especially for rigorous model selection in cosmology
        \end{block}
    \end{columns}
\end{frame}

\begin{frame}
    \frametitle{Bayesian Anomaly Detection for Type Ia Supernovae \arxiv{2509.13394}}
...
\begin{frame}
    \frametitle{Dark Energy vs Supernova Systematics \arxiv{2509.13220}}
...
\begin{frame}
    \frametitle{GPU Gravitational Wave Inference \arxiv{2509.04336}}
