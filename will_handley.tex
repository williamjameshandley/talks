...
\begin{frame}<1-2>
    ...
    \begin{columns}[T]
        ...
        \item  \textbf{Occam's Razor}: Automatic complexity penalty
            \[\log \mathcal{Z} = \langle \log \mathcal{L} \rangle_{\pi} - \mathcal{D}_{\text{KL}}\]
...
\begin{frame}
    ...
    \begin{alertblock}{Key Parameters}
        \begin{itemize}
            ...
            \item Examples: \texttt{MultiNest} efficiency \texttt{eff}, \texttt{PolyChord} chain length \texttt{n\_repeats}
        \end{itemize}
    \end{alertblock}
\end{frame}
