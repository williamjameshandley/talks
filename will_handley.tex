...
\begin{frame}
    ...
    %@claude -- some llm-isms have crept in here -- the tone should be slightly more sober -- 'no evidence calculation' repeated is too on-the-nose [check previous presentations for how this is phrased -- there is a david mackay quote that is relevant, that you should be able to find with an appropriate git search through will_handley.tex across branches].
    \begin{columns}[T]
        \column{0.48\textwidth}
        \begin{block}{MCMC (e.g. \texttt{emcee})}
            \begin{itemize}
                ...
                \item \textcolor{red}{No evidence calculation}
            \end{itemize}
        \end{block}
        ...
        \end{block}
        \begin{alertblock}{Key Message}
            If you need to compare physical models, you \textit{must} compute the evidence. Nested sampling is the go-to method.
        \end{alertblock}
    \end{columns}
\end{frame}
...
\begin{frame}
    ...
    %@claude -- again 'go-to' method too on-the-nose, remove key message, and get some material about volume/DKL estimation, again check through past presentations for similar material
    \begin{columns}[T]
        \column{0.5\textwidth}
        \vspace{-10pt}
        \only<1-8>{
...
\begin{frame}
    ...
    %@claude -- MCMC == parallel chains isn't quite right here -- this can only in principle parallesise up to 8, so we shouldn't be saying this! talk about emcee instead, and point out that traditional algos like HMC or MCMC can't exploit this
    \begin{columns}
...
%@claude -- should also include a slide on where we're going with this. This is with lawrence berry. You can find an email in my inbox with his picture, which you should add to this repo [and to the master branch -- commit any changes before switichng to adjust master]. Lawrence berry is currently working with myself, oli hahn and nils schoenberg to build a full GPU boltzmann solver [DISCO-DJ → DISCO-CL -- do some research.
\begin{frame}
