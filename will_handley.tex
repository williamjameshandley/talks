...
\begin{frame}
    ...
    \newlength{\challengeheight}\setlength{\challengeheight}{4.2cm}
    \begin{columns}[T]
        \column{0.32\textwidth}
        ...
        \begin{block}{\keyterm{Degenerate}}
            ...
            \end{itemize}
            \vspace{5pt}
            \textit{Challenge}: Different parameters produce similar observations}
        \end{block}
        \column{0.32\textwidth}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Why This Works}
            \begin{itemize}
                ...
                \item Estimates \keyterm{volume} probabilistically --- the only way in high dimensions.
                \item Computes \C[3]{evidence $\mathcal{Z}$} and \keyterm{information} $\mathcal{D}_\mathrm{KL}$:
            \end{itemize}
            \[\C[3]{\log\mathcal{Z}} = \av[{\C[0]{\mathcal{P}}}]{\C[2]{\log\mathcal{L}}} - \mathcal{D}_\text{KL} \sim \log\frac{\text{\C[1]{Prior volume}}}{\text{\C[0]{Posterior volume}}}\]
        \end{block}
        }
...
\begin{frame}
    \frametitle{GPU-Accelerated Applications: Cosmology}
    \framesubtitle{CMB and Cosmic Shear \arxiv{2509.13307}}
...
\begin{frame}
    ...
    \begin{columns}[c]
        ...
        \begin{alertblock}{Current Work}
            Extending to CMB-ready $C_\ell$ calculations with Oli Hahn \& Nils Schöneberg
        \end{alertblock}
        \column{0.4\textwidth}
        \centering
        \textit{[DISCO-CL figure placeholder]}
    \end{columns}
\end{frame}

\begin{frame}
    \frametitle{GPU-Accelerated Applications: Supernovae \& Gravitational Waves}
    \begin{columns}[T]
        \column{0.48\textwidth}
        \student{sam_leeney}{Sam Leeney}{PhD}
        \begin{block}{Type Ia Supernovae \arxiv{2509.13394}}
            \begin{itemize}
        ...
        \vspace{5pt}
        \centering
        \includegraphics[width=0.8\textwidth]{figures/2509.13394_19ekb_light_curves_all_paper.png}
        \column{0.48\textwidth}
        \student{metha_prathaban}{Metha Prathaban}{PhD}
        \begin{block}{Gravitational Waves \arxiv{2509.04336}}
            \begin{itemize}
        ...
        \vspace{5pt}
        \centering
        \includegraphics[width=0.8\textwidth]{figures/2509.04336_walltime_speedup.pdf}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[c]
        ...
        \begin{itemize}
            ...
            \item The ``80/20'' rule of scientific work:
                \begin{itemize}
                    \item 20\% ``hard thinking'' (physics, insight)
                    \item 80\% ``boring tasks'' (coding, debugging, formatting, data munging)
                \end{itemize}
            \item LLMs are not about ``automating science'' (the 20\%)
            \item They are about collapsing the 80\% of overhead
            \item Much of the BlackJAX nested sampling implementation was ``vibe-coded'' with LLM assistance
        \end{itemize}
        \column{0.4\textwidth}
        \centering
...
\begin{frame}
    \frametitle{Case Study: AI-Assisted Development}
    \framesubtitle{Examples from my group}
    \begin{columns}[c]
        \column{0.55\textwidth}
        \begin{block}{Cosmosis to Cobaya}
            \begin{itemize}
                \item Task: Port a legacy likelihood
            ...
            \end{itemize}
        \end{block}
        \begin{block}{jim-emulators (with James Alvey)}
            \begin{itemize}
                \item Goal: JAX emulator infrastructure for GW
...
\end{frame}
    ...
    %@claude -- I've been reminded of another case study, where I vibe coded emcee into blackjax in 15 minutes for a talk. We should mention this. The framework was that I was giving a talk where I wanted an equivalent plot for emcee as I did for nested sampling, but it was challenging. I therefore gave claude code the tex of the talk, the tex of the emcee paper, the github emcee repository, the blackjax emcee repository, and asked it to put emcee into blackjax. There is a pull request on blackjax-devs/blackjax which should include that that you can refer to on this extra slide. This was with opus 4.2 [I think -- it would have been the time I was giving the sed fitting talk -- you can get the date from the github repo]
